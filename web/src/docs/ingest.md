...
Full-content hydration prefers substantive feed-native HTML (`content:encoded`) and converts it to safe Markdown. If that is absent or too thin, Utopia fetches the first alternate HTTP(S) article link through a bounded SSRF-resistant client, applies readability extraction, and converts the result to Markdown. A failed linked fetch or summary-only entry stays retryable or terminal with a typed diagnostic; it is never marked as completed full content. Every entry has a visible pending, queued, retrying, complete, or terminal outcome; the current-generation details are available from the Entry diagnostics link.
