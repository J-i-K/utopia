...
| 0022 | [An unknown date is not an open one](0022-an-unknown-date-is-not-an-open-one.md) | First cut implemented · `world_axis` predicate beside `record_axis`, `facts.attested_at` anchors a missing start or an undated end at the document that attests it, every server read and both client filters on the read interval, an undated ending closes the dated row it ends · derived rows on read intervals are the second cut · a bare open row plus an undated ending is #393 |
