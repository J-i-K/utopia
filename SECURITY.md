...
Background extraction and entity adjudication may use the Codex Responses transport, but it is **disabled by default**. Interactive chat, chat tools, ontology/type-resolution work, and embeddings remain on their existing Chat Completions or embeddings paths.

The Codex capability is deployment-wide, while the transport choice is workspace-scoped in the existing Models selector. When the explicit `docker-compose.codex.yml` overlay is enabled, a workspace administrator may choose OpenAI API / Chat Completions or ChatGPT subscription / Responses for that workspace's background extraction and adjudication. The subscription credential belongs to the deployment and is never entered into or returned from the workspace settings. The selector is accepted only for the exact `https://api.openai.com` host; other OpenAI-compatible endpoints remain on Chat Completions. Enable the capability only with `bash scripts/utopia-codex-compose.sh`; the wrapper rejects relative host paths and paths resolving inside the repository before invoking Compose. Native Compose only provides the nonempty-variable check. Use a dedicated Utopia-owned `CODEX_HOME` outside the repository. Do not mount an operator's normal `~/.codex` profile or put token material in `.env`, Postgres, logs, backups, or source control.
